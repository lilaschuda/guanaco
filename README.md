...
If none of `routes.json`, `routes.yaml`, or `routes.yml` exists on the classpath, Guanaco falls back to scanning a `routes/` directory of multiple files instead — no separate flag required to opt in; a deployment chooses the convention simply by which one it has. Every file in the directory is parsed and merged into a single combined configuration, one key at a time, all the way down: a `framework:` block in one file and `routes:` entries in another merge cleanly, two files each declaring different fields of the same route entry merge cleanly, but the **same key declared in more than one file is always a hard, boot-time failure** — Guanaco never guesses at precedence or silently prefers one file over another. File discovery order has no bearing on the result: the merge is symmetric, and a collision is reported the same way, naming both files involved, regardless of which one happened to be read first.

Within one scanned directory, the same JSON-over-YAML precedence used for single-file loading applies again, independently, per logical file name (e.g. `payments.yaml` and `payments.json` in the same directory — JSON wins for `payments` specifically; an unrelated `orders.yaml` with no JSON counterpart is unaffected). Whether an entire directory is allowed to mix JSON and YAML/YML files *at all* is a separate, coarser policy: by default it is not — a directory containing both fails to load, with a clear error explaining why. Set the `guanaco.config.allowMixedFormats` system property to `true` to permit it, e.g. for an in-progress migration between formats, or a mix of hand-authored and generated files.

The scanned directory itself defaults to `routes/`, overridable via the `guanaco.routes.dir` system property — e.g. to point at a different directory per environment, such as a hand-written YAML directory for local development and a generated JSON directory for deployment.
...
Guanaco depends on `camel-spring-xml` and `spring-context` to support loading legacy Spring-based Camel XML routes alongside Guanaco-managed routes in the same context — this is what powers incremental migration from an existing Camel deployment. Multi-file configuration scanning (see [Configuration format](#configuration-format)) also relies on Spring's classpath resource resolution, for the same underlying reason: `GuanacoContext` is already built on `SpringCamelContext`, so reusing Spring's own resource-scanning keeps the codebase consistent with what it already is, rather than avoiding Spring in one place while remaining fully dependent on it everywhere else. If you're starting a greenfield project with no existing Camel XML routes, these transitive dependencies are present but unused. A lighter-weight variant without Spring may be offered in a future version once there's clearer evidence of demand for it.
